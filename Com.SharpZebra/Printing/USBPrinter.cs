...
namespace Com.SharpZebra.Printing
{
    ...
    public class UsbPrinterConnector
    {
        ...
        public static Dictionary<string, string> EnumDevices()
        {
                ...
                foreach (string printer in nameKey.GetSubKeyNames())
                {
                    try
                    {
                        using (var subKey = Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\Print\\Printers\\" + printer))
                        {
                            ...
                            if (objPortValue == null) continue;//If it is null then don't move forward with it as .ToString() Raises Object Reference Not Set to an Instance Error.
            ...
            using (RegistryKey regKey = Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\DeviceClasses\\" + GUID_DEVICEINTERFACE_USBPRINT.ToString("B"), false))
            {
                foreach (string sub in regKey.GetSubKeyNames())
                {
                    try
                    {
                        ...
                        {
                            using (var subKey = Registry.LocalMachine.OpenSubKey(string.Format("SYSTEM\\CurrentControlSet\\Control\\DeviceClasses\\{0:B}\\{1}\\#", GUID_DEVICEINTERFACE_USBPRINT, sub)))
                            {
                                var objSymbolicLink = subKey.GetValue("SymbolicLink");
                                if (objSymbolicLink == null) continue; //If it is null then don't move forward with it as .ToString() Raises Object Reference Not Set to an Instance Error.
                                path = objSymbolicLink.ToString();
                                subKey.Close();
                            }

                            using (var subKey = Registry.LocalMachine.OpenSubKey(string.Format("SYSTEM\\CurrentControlSet\\Control\\DeviceClasses\\{0:B}\\{1}\\#\\Device Parameters", GUID_DEVICEINTERFACE_USBPRINT, sub)))
                            {
                                var objPortNumber = subKey.GetValue("Port Number");
                                if (objPortNumber == null) continue;//If it is null then don't move forward with it as .ToString() Raises Object Reference Not Set to an Instance Error.
